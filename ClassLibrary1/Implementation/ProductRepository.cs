...
namespace Shop.Dal.Implementation
{
	...
	public class ProductRepository : IProductRepository
	{
		...
		public List<Product> GetProducts(GetProductsRequest request)
		{
			...
			{
				var query = "SELECT p.Id, p.Name, p.Type, p.Material, pp.Price, pph.FileName, p.Description,s.Name as Size, p.Articul" +
		...
		public ProductCard GetProductCard(int id)
		{
			using(var connection = GetConnection())
			{
				...
				product.ImageNames = connection.Query<string>($"SELECT FileName FROM [dbo].[ProductPhotos] with(nolock) where productId = {id}", commandType: CommandType.Text).ToList();

				var priceQuery = $"SELECT pp.Price,pp.SizeId,s.Name as SizeName FROM [dbo].[ProductPrice] pp with(nolock) JOIN [dbo].[Sizes] s with(nolock) ON pp.SizeId=s.Id WHERE pp.ProductId={id}";
